...
输入：BookShelf 格式文件中的电路放置单元信息，包括单元放置位置以及引脚连接等。
...
1. 计算前瞻梯度：$g_{t}=\nabla f\left(\theta_{t}-\beta v_{t-1}\right)$（沿动量方向"预走"一步，即前瞻点）
2. 计算动量：$v_{t}=\beta v_{t-1}+(1-\beta) g_{t}$
3. 更新参数：$\theta_{t+1}=\theta_{t}-\alpha v_{t}$

注：上述是 NAG 的通用形式。文献[1]（ePlace）实际采用的 Nesterov 变体略有差别：步长取 Lipschitz 常数的动态预测值的倒数 $\alpha_k = 1/L_k$，加速因子按 $a_{k+1}=\left(1+\sqrt{4 a_k^2+1}\right)/2$ 递推，迭代式为

$$v_{k+1}=u_k-\alpha_k \nabla f(u_k), \qquad u_{k+1}=v_{k+1}+\frac{a_k-1}{a_{k+1}}\left(v_{k+1}-v_k\right)$$

其中 $u_k$ 为参考解（前瞻点），$v_k$ 为当前解，位置的更新在参考解上进行。具体实现时以文献[1]第 5 节为准。
...
1-5. 惩罚系数 lambda 初始化（penaltyFactorInitilization）

计算初始 HPWL；累加全部节点的线长梯度绝对值之和（numerator）；累加全部节点 + filler 的密度梯度绝对值之和(denominator)；令 lambda= numerator / denominator，得到 lambda 初始值，平衡两项梯度量级。
...
（2）密度溢出率 τ 计算方法（对应文献[1]式 (37)）：

$$\tau=\frac{\sum_{b \in B} \max \left(\rho_{b}^{\prime}-\rho_{t},\ 0\right) \cdot A_{b}}{\sum_{i \in V_{m}} A_{i}}$$

其中，

- $\rho_{b}^{\prime}$ 为网格 b 中仅由可移动单元（标准单元与宏单元，宏单元面积按 targetDensity 缩放后计入）产生的密度，B 为所有划分网格，$A_{b}$ 为网格 b 的面积；
- 分母 $\sum_{i \in V_{m}} A_{i}$ 为可移动单元的（缩放）总面积，即 标准单元总面积 + 宏单元总面积 × targetDensity；
- 单网格的溢出必须取正部 $\max(\rho_{b}^{\prime}-\rho_{t}, 0)$：否则稀疏网格的"负溢出"会抵消拥挤网格的正溢出；且由于单元总面积守恒，不取正部时 τ 将近似为常数，失去作为停止判据的意义。

注意：终端（固定单元）与暗节点不计入溢出统计。
...
其中 $q_{i}$ 为单元 i 的电荷大小，等于单元 i 的面积，是已知输入；布局区域被划分为 m × m 个网格，对每个网格内各单元（标准单元、宏单元、填充单元等）的重叠面积求和，得到该网格的电荷密度 ρ。$\xi_{x}$ 为电场 ξ 在 x 方向值，$\xi_{y}$ 为电场 ξ 在 y 方向值。

电场 $\xi_{x}$、$\xi_{y}$ 计算公式为（即式（10），注意 ξ 是电场而非电势梯度，$\boldsymbol{\xi}=-\nabla\psi$）：
...
```cpp
bins[i][j]->fillerDensity += LengthScale.x * LengthScale.y * overlapArea;
```

注：密度缩放（乘 targetDensity）只施加于宏单元、固定终端与暗节点（见文献[1] 3.2 节 Density scaling）；填充单元的总面积本身就是按目标密度设计的（$A_{fc} = \rho_t A_{ws} - A_m$），其密度贡献不再乘 targetDensity。
...
其中，总单元面积 = `bins[i][j]->nodeDensity` + `bins[i][j]->baseDensity` + `bins[i][j]->fillerDensity` + `bins[i][j]->terminalDensity`，`baseDensity` 与 `terminalDensity` 在初始化时求得，详见本文 3.2 节「1. 初始化阶段」之 **1-2 密度 Bin 网格初始化（BinInit）**。

> **标注（整理者注）**：原文此处有两处问题，均已修正。
>
> 1. **交叉引用错误**：原文写"详见初始布局设计 3.2 节"，指向了《4 初始布局设计》，但该文并无相关内容。`baseDensity` 与 `terminalDensity` 的计算方法实际就定义在**本文**的 3.2 节 1-2 BinInit 中。
> 2. **命名不一致**：原文此处写作 `darkDensity`，而本文 1-2 BinInit 及后续实现（含参考实现 RePlAce / easyPlace）统一使用 `baseDensity`。二者指同一个量——bin 内不可放置区域（即 SiteRow 之外的区域，对应《2 布局基础》中的"暗节点"概念）折算出的等效密度。此处统一为 `baseDensity`。

**步骤 1-4-2-3：计算 DCT 系数 $a_{j, k}$**
...
其中 $\mu_{k}=1.1^{-\left(\Delta \mathrm{HPWL}_{k} / \Delta \mathrm{HPWL}_{\mathrm{REF}}\right)+1.0}$；$\Delta \mathrm{HPWL}_{\mathrm{REF}}=3.5 \times 10^{5}$，$\Delta \mathrm{HPWL}_{k}=\mathrm{HPWL}\left(\vec{v}_{k}\right)-\mathrm{HPWL}\left(\vec{v}_{k-1}\right)$。
