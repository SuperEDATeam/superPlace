...
std::unique_ptr<IInitialPlacer> makeInitialPlacer(const std::string& method) {
    ...
    if (method == "cluster_fc")
        return std::make_unique<ClusterPlacer>(makeFirstChoiceMatcher(), "cluster_fc");
    if (method == "cluster_bc")
        return std::make_unique<ClusterPlacer>(makeBestChoiceMatcher(), "cluster_bc");
